...
| **Configurable shortcut** | Closed. The shortcut is a persisted `NotchClipHotKeyBinding` (Carbon key code + modifiers, `com.anoop.notchclip.hotKey`, default ⌃V). Settings › General › Shortcut records a new one, Core validates it (at least one of ⌃⌥⌘ unless it is a function key), and the coordinator re-registers atomically — rolling back to the last working binding and surfacing the existing error label when Carbon reports the combo already taken. | Done |
| **⌘1–⌘9 direct select** | Closed. ⌘1–⌘9 pastes the Nth visible row; filters moved to ⌥1–⌥6. This is the reverse of the assignment sketched here earlier — ⌘N-activates-the-Nth-item is the platform idiom, and the filter already has two visible affordances while quick paste's only affordance is the shortcut. | — |
| **Paste as plain text** | Closed. ⇧⏎ pastes the selection as plain text, with an `alwaysPastePlainText` preference that the modifier inverts. `PlainTextPastePolicy` picks the source representation; images and file lists fall back to a normal paste. | — |
